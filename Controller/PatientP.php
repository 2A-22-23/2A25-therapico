...
class patientP
{
    ...
    {
        $sql = "SELECT cin_p,nom_prenom_p,date_n_p,tel_p,email_p,sexe_p,pass_p FROM patient";
        $db = config::getConnexion();
        try {
            $liste = $db->query($sql);
            return $liste;
 ...
 function searchPatientsById($cin_p){
    try {
        $db = Config::getConnexion();
        $query = $db->prepare('SELECT * FROM patient WHERE cin_p = :cin_p');
